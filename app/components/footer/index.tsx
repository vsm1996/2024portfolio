...
const Footer = () => {
  return (
    <div className='w-5/12 md:w-2/12 flex items-center justify-around mt-7'>
      <Link
        className='group relative p-6 hover:animate-bounce ease-in-out duration-300 transition-all'
        target='_blank'
        href={'https://www.linkedin.com/in/vsm1996'}
        aria-label="Link to Vanessa's LinkedIn Profile"
      >
        <Image width={63} src={LinkedInLogo} alt='linkedin logo' className='ease-in-out duration-300 group-hover:scale-125 group-active:scale-100' />
      </Link>
      <Link
        className='group relative p-6 hover:animate-bounce ease-in-out duration-300 transition-all'
        target='_blank'
      ...
      >
        <Image width={63} src={GithubLogo} alt='github logo' className='ease-in-out duration-300 group-hover:scale-125 group-active:scale-100' />
      </Link>
    </div>
  )
}
